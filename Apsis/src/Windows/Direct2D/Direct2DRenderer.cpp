...
namespace A {
	...
	void Direct2DRenderer::DrawRectImpl(const Eigen::Vector2i& position, float width, float height, float angle, Shared<Texture> texture, const Eigen::Vector4f& col/*= {1.0f, 1.0f, 1.0f, 1.0f}*/)
	{
		AP_PROFILE_FN();
		m_SolidColorBrush->SetColor(D2D1::ColorF(col.x(), col.y(), col.z()));
