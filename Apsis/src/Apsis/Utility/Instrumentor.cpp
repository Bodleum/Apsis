...
namespace A {
	...
	void Instrumentor::BeginSession(const std::string& name, const std::string& filepath/* = "results.json" */)
	{
		...
		if (m_CurrentSession)
		{
			...
			{
				AP_ERROR_C("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
			}
			InternalEndSession();
		}
		...
		else
		{
			if (Logger::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
			{
				AP_ERROR_C("Instrumentor could not open results file '{0}'.", filepath);
			}
		}
	}
